...
export class EventService {
  ...
  protected convertDateFromClient(event: IEvent): IEvent {
    const copy: IEvent = Object.assign({}, event, {
      dateCreated: event.dateCreated != null && event.dateCreated.isValid() ? event.dateCreated.toJSON() : null,
      dateExpired: event.dateExpired != null && event.dateExpired.isValid() ? event.dateExpired.toJSON() : null
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      ...
      res.body.dateExpired = res.body.dateExpired != null ? moment(res.body.dateExpired) : null;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((event: IEvent) => {
        event.dateCreated = event.dateCreated != null ? moment(event.dateCreated) : null;
        event.dateExpired = event.dateExpired != null ? moment(event.dateExpired) : null;
      });
    }
    return res;
  }
